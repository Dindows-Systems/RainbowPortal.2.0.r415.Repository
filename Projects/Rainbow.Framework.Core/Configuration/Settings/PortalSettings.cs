	...
	public class PortalSettings
	{
		...
		public static Hashtable GetPortalCustomSettings(int portalID, Hashtable _baseSettings)
		{
			if (!CurrentCache.Exists(Key.PortalSettings()))
			{
				...
				//Rainbow.Framework.Settings.Cache.CurrentCache.Insert(Rainbow.Framework.Settings.Cache.Key.PortalSettings(), _baseSettings);
				CacheDependency settingDependencies =
					new CacheDependency(
						null,
						new string[]
				...
				using (SqlConnection myConnection = Config.SqlConnectionString)
				{
					using (SqlCommand myCommand = new SqlCommand("rb_GetPortalSettingsLangList", myConnection))
					{
						...
						catch (Exception ex)
						{
							...
							Rainbow.Framework.ErrorHandler.Publish(LogLevel.Warn, "Failed to get languages from database.", ex); // Jes1111
						}
		...
		[Obsolete("Please use Rainbow.Framework.Settings.Database.DatabaseVersion", false)]
		...
		[Obsolete("Please use Rainbow.Framework.Settings.Config.SqlConnectionString")]
