<%@ control autoeventwireup="false" codefile="Events.ascx.cs" inherits="Rainbow.Content.Web.Modules.Events"
    language="c#" %>
    ...
    <itemtemplate>
        <span class="ItemTitle">
            <rbfwebui:hyperlink id="editLink" runat="server" imageurl='<%# CurrentTheme.GetImage("Buttons_Edit", "Edit.gif").ImageUrl %>'
                navigateurl='<%# Rainbow.Framework.HttpUrlBuilder.BuildUrl("~/DesktopModules/CommunityModules/Events/EventsEdit.aspx","ItemID=" + DataBinder.Eval(Container.DataItem,"ItemID") + "&mid=" + ModuleID) %>'
                visible="<%# IsEditable %>">
            </rbfwebui:hyperlink>
