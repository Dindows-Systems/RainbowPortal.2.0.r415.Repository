			...
			if (Page.IsValid)
			{
				...
				catch(Exception ex)
				{
					...
					while (ex != null)
					{
						...
						Rainbow.Framework.Helpers.LogHelper.Log.Error("Installing: ", ex);
		...
		protected override void OnDelete(EventArgs e)
		{
			try
			{
				if (!btnUseInstaller.Visible)
					Rainbow.Framework.Helpers.ModuleInstall.UninstallGroup(Server.MapPath(PortalSettings.ApplicationPath + "/" + InstallerFileName.Text));
				else
					Rainbow.Framework.Helpers.ModuleInstall.Uninstall(DesktopSrc.Text, MobileSrc.Text);
			...
			{
				Rainbow.Framework.Helpers.LogHelper.Log.Error("Error deleting module", ex);
