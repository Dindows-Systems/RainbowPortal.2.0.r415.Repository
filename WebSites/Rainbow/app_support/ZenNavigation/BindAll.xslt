...
<xsl:stylesheet version="1.0" exclude-result-prefixes="rainbow" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:rainbow="urn:rainbow">
	...
	<xsl:param name="UsePathTraceInUrl">true</xsl:param>
	<xsl:param name="UsePageNameInUrl">true</xsl:param>
	<!--	<xsl:variable name="myBranch" select="/MenuData/MenuGroup/MenuItem[descendant-or-self::MenuItem[@ID = $ActivePageId]]"/>-->
	<xsl:variable name="myBranch" select="//MenuItem[@ParentPageId = '0']"/>
	...
	<xsl:template match="MenuItem">
		...
		<xsl:choose>
			<xsl:when test="rainbow:CheckRoles(string(@AuthRoles))">
				<xsl:element name="li">
					<xsl:element name="a">
						<!--xsl:attribute name="href"><xsl:value-of select="rainbow:BuildUrl(string(@PageName),number(@ID))"/></xsl:attribute-->
						<xsl:choose>
							<xsl:when test="$UsePageNameInUrl='true' and $UsePathTraceInUrl='true'">
								<xsl:attribute name="href"><xsl:value-of select="rainbow:BuildUrl(string(@UrlPageName),number(@ID),$pathtrace)"/></xsl:attribute>
							</xsl:when>
							...
							<xsl:when test="$UsePageNameInUrl='false' and $UsePathTraceInUrl='true'">
								<xsl:attribute name="href"><xsl:value-of select="rainbow:BuildUrl(number(@ID),$pathtrace)"/></xsl:attribute>
							</xsl:when>
							<xsl:otherwise>
